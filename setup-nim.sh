...
Login/Register for a free account (includes 1,000 free credits).
Select Qwen 3 Coder 480B (or your preferred model).
Click Get API Key.
Copy the key and paste it when prompted by the CLI.
Disclaimer: This is an independent patch and is not affiliated with Anthropic or NVIDIA.

---
#### 2. File: `setup-nim.sh`
Copy this content into a file named `setup-nim.sh`.
```bash
#!/bin/bash
